...
struct Building: Codable {
    ...
    var density_cnt: String
    var capacity: String
    var dc_ratio: String
    var floors: Array<String>
    var remaining_dc_ratio: String
}
...
struct ListUIView: View {
    ...
    func loadData(){
        let task = URLSession.shared.dataTask(with: URL(string:"https://buspot.herokuapp.com/getbuildingdensity/733")!, completionHandler: {data, response, error in
            ...
            var result: Building?
            do {
                result = try JSONDecoder().decode(Building.self, from: data)
            }
            catch {
